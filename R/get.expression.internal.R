get.expression.internal <- function(x, primes, prime.counter, start.sum) {
  P <- ""
  ...
  if (x$fraction) { 
    P <- paste0(P, "\\frac{", get.expression.internal(x$num, primes, prime.counter, TRUE), 
      "}{", get.expression.internal(x$den, primes, prime.counter, TRUE), "}",collapse = "")
  }
  if (x$sum) {
    ...
    for (i in 1:length(x$children)) {
      add.strings[i] <- paste0(c("w_{", i, "}^{(", x$weight, ")}", 
        get.expression.internal(x$children[[i]], primes, prime.counter, TRUE)), collapse = "")
    }
  ...
  if (!(x$sum || x$product || x$fraction)) {
    ...
    if (x$domain > 0) {
      if (x$domain == 1) P <- paste0(P, "^*(", var.string, collapse = "")
