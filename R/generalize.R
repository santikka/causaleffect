generalize <- function(y, x, Z, D, expr = TRUE, simp = FALSE, steps = FALSE, primes = FALSE, prioritize = FALSE, weighted = TRUE) {
  d <- length(D)
  z <- length(Z)
  ...
  res <- trmz(y, x, probability(domain = 1), c(), 1, 1, D, Z, topo, list(), prioritize, weighted)
  res.prob <- res$P
